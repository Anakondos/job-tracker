"""
Smart Filler V3.4 - Pre-Scan Strategy

NEW: Pre-scan all autocomplete fields BEFORE filling:
1. Open each dropdown, read available options
2. Classify: FIXED (≤20 options) vs SEARCHABLE (>20 or dynamic)  
3. For FIXED: find exact match from our defaults
4. For SEARCHABLE: use type-to-filter strategy

Plus all V3.3 features:
- Cascade detection (HTML → ARIA → known → label)
